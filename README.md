...
- 医嘱与处方管理：在病历时间轴选中病历后开方（药品、规格、用量、频次、疗程），患者过敏史命中药品名自动拒绝；同一病历相同药品在待审核/已审核阶段只保留一条，重复提交返回已有处方；处方按待审核→已审核→已执行依次流转，越级操作被拒绝，每次变化记录操作人与时间并可查看流转记录。
...
│   ├── src/prescriptions/ # 处方开具、去重与状态流转
